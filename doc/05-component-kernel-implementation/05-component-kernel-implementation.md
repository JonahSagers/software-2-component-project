...
At its core, OuijiBoard is just a sequence of characters. Only one is required
to be accessible at any given time. Of the OSU CSE components, queue fits this
the best since it only exposes the required letter and can quickly cycle to the
next letter in the spirit. However, a string provides the exact same utility.
Although it does expose more than just the required letter, that isn't ever an
issue since the spirit isn't directly accessible to the user anyways.

For a string implementation, it is required that index is always less than the
length of the spirit, and that spirit has a length > 0.
